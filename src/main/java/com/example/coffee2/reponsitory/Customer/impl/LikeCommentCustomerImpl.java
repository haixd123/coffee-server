...
import com.example.coffee2.response.TableLikeCommentResponse;
...
//        if (!isCount) {
//            sql.append(" select \n" +
//                    " lc.comment_id as lcCmmentId, lc.user_id as lcUserId, c.like_comment, \n" +
...
//            }
//            sql.append(" group by c.post_id, c.id, c.user_id, c.comment_text, c.create_at, lc.comment_id, lc.user_id, c.like_comment " +
...
//    }
    ...
    public List<LikeCommentResponse> getTableLikeCommentListLikeComment(LikeCommentRequest request) {
        try {
            StringBuilder sql = new StringBuilder();
            Map<String, Object> params = new HashMap<>();
            createSqlTableGetListLikeComment(request, sql, params, false);
            Query query = entityManager.createNativeQuery(sql.toString());
            if (params.size() > 0) {
                params.forEach((key, value) -> {
                    query.setParameter(key, value);
            ...
            return FunctionUtils.mapping(query.getResultList(), TableLikeCommentResponse.class);
        } catch (Exception e) {
            log.error("error1: " + e.getMessage());
        }
        return null;
    ...
    private void createSqlTableGetListLikeComment(LikeCommentRequest request, StringBuilder sql, Map<String, Object> params, boolean isCount) {
//        if (!isCount) {
            sql.append(" select comment_id, user_id from like_comments ");
//            if (request.getPostId() != null) {
//                sql.append("and c.post_id = :postId \n");
//                params.put("postId", request.getPostId());
//            }
