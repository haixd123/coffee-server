...
//        }
    ...
    public List<PostsResponse> getListPostsAdmin(PostsRequest request) {
        try {
            StringBuilder sql = new StringBuilder();
            Map<String, Object> params = new HashMap<>();
            createSqlGetListPostsAdmin(request, sql, params, false);
            Query query = entityManager.createNativeQuery(sql.toString());
            if (params.size() > 0) {
                params.forEach((key, value) -> {
                    query.setParameter(key, value);
    ...
    public Long getCountListPostsAdmin(PostsRequest request) {
        try {
            StringBuilder sql = new StringBuilder();
            Map<String, Object> params = new HashMap<>();
            createSqlGetListPosts(request, sql, params, true);
...
//            }
    ...
    private void createSqlGetListPostsAdmin(PostsRequest request, StringBuilder sql, Map<String, Object> params, boolean isCount) {
        log.info("request: " + request);
        if (isCount) {
            sql.append("select count(*) ");
        } else {
